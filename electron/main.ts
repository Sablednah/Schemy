...
function fileFromArgs(args: string[]) { return args.find(a => /\.(schematic|schem|nbt|litematic)$/i.test(a)); }
...
if (!gotLock) app.quit();
  ...
  app.whenReady().then(() => {
    ipcMain.handle('choose-file', async () => (await dialog.showOpenDialog({ properties: ['openFile'], filters: [{ name: 'Minecraft Structures', extensions: ['schematic', 'schem', 'nbt', 'litematic'] }] })).filePaths[0]);
